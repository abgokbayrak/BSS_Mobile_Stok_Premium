...
class ImalattanIadeBarkodSearchModal extends StatefulWidget {
  final barkodID;
  final olukluDepoID;
  ImalattanIadeBarkodSearchModal(
      {Key? key, required this.barkodID, required this.olukluDepoID})
      : super(key: key);

  @override
  _ImalattanIadeBarkodSearchModalState createState() =>
      _ImalattanIadeBarkodSearchModalState();
}

class _ImalattanIadeBarkodSearchModalState
    extends State<ImalattanIadeBarkodSearchModal> {
  ...
  getBarkodsForSecondButton() async {
    EasyLoading.show(status: "Barkodlar Yükleniyor...");
    barkodAfterOlukluSelectedItems.clear();
    var dbClient = await db;
    var query =
        "SELECT Stok_Karti_Barkod.BarkodID,Depo_Hareketleri.LotNo FROM Depo_Hareketleri INNER JOIN Stok_Karti_Barkod ON Stok_Karti_Barkod.BarkodID = Depo_Hareketleri.LotNo WHERE (Depo_Hareketleri.AmbarID = ${widget.olukluDepoID}) AND Stok_Karti_Barkod.Durum = 0 AND Depo_Hareketleri.Durum = 0 AND Depo_Hareketleri.HareketID IN (9,17) GROUP BY Depo_Hareketleri.LotNo,Stok_Karti_Barkod.BarkodID";
